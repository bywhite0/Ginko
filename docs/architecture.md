...
当前实现数据契约、存储原语及可注入决策/发送函数的单实例异步运行链。
本机反向 WebSocket 与有预算约束的 HTTP 模型入口已测试，真实模型、人格决策及生产发送策略
尚待联调，CLI 暂不提供聊天运行命令。
...
当前仅允许本机 OneBot 监听，主动活动保持关闭；混合响应模式仅接受白名单私聊与定向群文本。
配置检查通过不代表协议端、模型或费用估算已在真实调用中验证。
...
这些测试采用合成决策，不表示真实模型或第三方测试协议端已经联调完成。
...
sender 在协议端在线时领取出站，并重新检查授权；超时、取消或不确定异常记录 unknown。
...
模型与发送函数必须遵守异步取消和有界请求契约；目前生命周期通过合成实现验证，真实实现随后接入。
...
输入全部按未缓存价格计算，输出包括推理 token，不假定缓存优惠。结算是本地保守费用核算，
...
当前选定端点只完成只读模型存在性核对；真实价格仍需账户信息，尚未进行付费调用。
...
下一阶段接入一个明确支持的模型端点、人格决策与生产投递策略，提供聊天运行命令。
先在一个专用测试会话验证文本应答，接着用第二平台的实际事件检查接口。
