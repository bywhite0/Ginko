...
`ginko backup <data_dir> <destination>` 与 `ginko restore <backup> <data_dir>` 先取得数据目录独占锁，
服务运行时直接拒绝。备份使用 SQLite 备份接口，包含崩溃后尚未合并的 WAL 已提交内容；
副本改为回滚日志模式，成为不依赖 `-wal` / `-shm` 的单一文件。两者都不迁移 schema，
旧版本数据库在 runtime 打开时才按现有事务迁移。

写入先落到 `.partial` 临时文件，完整性检查及状态摘要与来源一致后才以不覆盖的方式发布。
目标文件或其 `-wal`、`-shm`、`-journal` 已存在时拒绝，避免旧 WAL 被重放到恢复出的库上；
恢复不合并数据，需要替换现有库时由维护者先移走原文件。输出的状态摘要包含 schema、
收件与出站各状态计数及最后序号、预算预留与结算合计及模型尝试状态，不含消息正文或身份标识。
恢复会重新复制页面，恢复后文件的 SHA-256 与备份文件不同，以摘要一致作为核对依据。

恢复不改变状态语义：`sent` 不回到待发，`unknown` 不被重新领取，预留预算不释放；
取得锁的 runtime 启动时照常执行中断恢复。恢复较早备份后，仍须按发布计划核对备份之后的
回执、未知投递和预算，不能以恢复替代账本核对。
...
提供方按时段变价时，`[[model.rate_windows]]` 以 UTC 星期和时间段声明各时段费率，
未被任何时段覆盖的时间使用基础费率。提供方可能按请求开始或完成时刻计价，
因此结算取请求开始到响应结束之间触及的全部时段，逐项取最高费率；
只有时段完整覆盖整个请求时才不计入基础费率。预留按基础费率与全部时段的逐项最高值计算，
时段变化不会造成超出预留。维护者核实的节假日写入 `excluded_dates`，
未列出的日期按所声明时段结算，本地记账只会偏高。价格表不内置、不在线拉取，
变化时由维护者更新费率与 `price_checked_on`，与账单的核对方式不变。
...
未配置 `persona_dir` 时加载包内吟子，并校验 `approved_persona_version`。配置 `persona_dir` 时，
从相对配置文件的本地目录加载同样结构的人格，manifest 必须声明 `change_policy = "maintainer_review"`，
资源只能是同目录文件名；还须配置 `approved_persona_sha256`。摘要覆盖 manifest 与三份资源文本，
不受换行风格影响；版本或摘要任一不符即拒绝启动，文件改动后需要重新审核并更新批准值。
`check-config` 输出当前人格的 id、版本与摘要，供维护者在审核后填写。
`agent_id` 必须等于人格 id：活动与后续记忆按 agent 隔离，一个 agent 只对应一个人格。

manifest 的 `relationship_terms` 列出只能对可信关系对象使用的称呼。决策提示词据此说明授权称呼；
未建立关系时，回复包含这些称呼会被拒绝并重试。吟子声明“花帆学姐”，提示词与此前逐字相同。
称呼本身与角色自称重合时无法由输出检查区分，只能依靠人格文本约束，例如花帆以第三人称自称“花帆”。
本地人格目录不进入构建产物，其来源与分发范围由维护者另行确认。
